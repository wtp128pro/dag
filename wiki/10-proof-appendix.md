...
> **Unchanged through plugin 1.10.1 / catalog 1.0.13.** The socratic-guardrail landing — six OFFLINE post-hoc invariants **I35–I40** (1.10.0) plus **I39-7** (1.10.1) — leaves `formal/Pipeline.tla`, `formal/Pipeline.cfg`, `formal/WorkGraph.als`, and `formal/Amendment.als` **byte-identical**, so it adds **no TLA action, no variable, and no back-edge**. Every predicate is offline post-hoc (writes no `retries`/`fuel`, gates no transition, never guards `LT7` — a live guard there would deadlock `RETRY`, breaking Property 2 Claim D), so `GateOrdering`/`Termination`/`Quiesce` hold verbatim and **I18 is carried VERBATIM**. The transcripts below (853 / 408 / depth 36; MaxFuel 32 → 2,923 / 1,608 / depth 156; Alloy 8/8) therefore stand **unchanged** — `run_formal.sh` was **NOT re-run** and is not required, since the counts are a function of the byte-identical model text. The one new amendment kind, `revise_anchors` (I40), is a fuel-1 sub-case of the modeled `Amend` action, already covered by `Quiesce` (`references/formal-models.md` §"Socratic-guardrail 1.10.0" + §"1.10.1 addendum").
