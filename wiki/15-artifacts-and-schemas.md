...
| # | Schema | Validates | Required keys | Notable constraint (1.3.0 / 1.7.0 / 1.8.0 / 1.9.0) |
|---|---|---|---|---|
...
*Machine-checked (in scope):* schemas 1–9, 11, and 13–15 are applied by `validate_run.py` over emitted artifacts (top-level: personas/clarifications/cartography/graph/fsm-state and — when source cartography exists — `sources.json` under the offline **I26** check; per-unit: brief/debrief/verify/disagreement; and — when amendments exist — `amendments/A<NN>.json` under the I17/I18/I19 offline checks, page 14 §4.2). Schema 10 (`persona`) is meta-validated only under `--self-check`. Schema 12 (`manifest`) is validated by the decomposer, not auto-run — see the last section.

### Deep dive — `fsm-state.schema.json`: the sign-off gate, per-unit loop state, the 1.7.0 stamps, and the 1.9.0 depth tier

Five schema additions live here — two from 1.3.0, two from the 1.7.0 BGA / audit-round-2 work, and one from the 1.9.0 depth-tier work — and all are worth pinning.

**`gates.signoff_confirmed` (D-06/BRK-13, 1.3.0).** The gate object grew a fifth flag. It is a human attestation the validator checks the *presence* of — REQUIRED `true` to reach phase `DONE`, because `validate_run.py`'s `REQUIRED_GATES` lists it for `DONE`, so **a run at `DONE` without the flag is INVALID** (fsm-state.schema.json:36-39; SKILL.md:804-806). The sign-off is set only after the human accepts at the Phase-8 gate — Dag sets `gates.signoff_confirmed = true` *before* advancing `phase` to `DONE` (SKILL.md:802-803). Like `personas_confirmed`, the validator checks that the flag is present, not whether the human genuinely accepted — a *machine-checked* presence gate over an *asserted* attestation.
...
**Optional `depth` run-tier block (1.9.0).** The state file MAY carry a `depth` object recording the human-gated *run depth tier* — `light`/`standard`/`full` — confirmed at the Phase-1 persona gate (fsm-state.schema.json:90-141). It **rides the existing persona gate — no new gate flag, `REQUIRED_GATES` untouched.** REQUIRED inside the block are `tier` (the EFFECTIVE tier), `proposed_tier`/`confirmed_tier` (the immutable gate record), `confirmed_at_gate`, and a `justification` carrying `stakes`, `reversibility`, an `external_surface`, and the canonical `skipped_floors` delta disclosure (`[]` only for `full`) (:94-118). The optional `phase2_touch` records the unconditional Phase-2 keep-or-raise touch (:119-124), and `overrides[]` is **append-only and UPWARD-only** (`light` < `standard` < `full`) — each entry carries a `decision_ref` + a `pending_units` snapshot, and a raised floor binds only units not yet PASS-verified (per-unit `tier_at_verification` in verify) (:125-140). An **absent** `depth` block ⇒ the mechanism is not adopted and the validator's I28 is silent (archive-safe). Only the depth-tier work writes it; `validate_run.py`'s I28 checks its shape, gate provenance, the canonical `skipped_floors` set, and the upward-only ratchet (page 14). Like the other blocks here it adds **no** FSM gate flag, so the gate-ordering guarantee is *preserved*.
...
- **Page 14 — Validator & invariants:** what `validate_run.py` *does* with these schemas (schema self-check, top-artifact + per-unit validation, and the I1–I34 / I1b–I1d / I3b / I3c / I-dod invariant catalog — including the BGA I17/I18/I19 amendment checks — that layers structural checks on top of shape).
