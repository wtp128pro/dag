...
Three of the four are now **mechanically backed** — the validator's `REQUIRED_GATES` enforces the gate-ordering flags `personas_confirmed` (Phase 1) and — new in 1.3.0 — `signoff_confirmed` at `DONE` (Phase 8, D-06), while the Phase-2 clarification gate is backed by the **separate artifact-driven `I-dod` check** (which demands a schema-valid `clarifications.json` carrying Definition-of-Done + Non-Goals whenever any post-clarification structural artifact exists — not a `REQUIRED_GATES` entry). **Phase 7's disagreement gate (G-resolve) is the sole purely-human gate the validator cannot check:** it can confirm a decision was *recorded* (an appended `DECISIONS.md`) but not that a human made it (state-machine.md:133-135). In every backed case only the *presence* of the flag is machine-checked; its *genuineness* stays a validity ≠ correctness limitation.
